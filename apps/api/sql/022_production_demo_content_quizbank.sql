CREATE TABLE IF NOT EXISTS quiz_bank_questions (
  ...
  question_number INTEGER NOT NULL CHECK (question_number BETWEEN 1 AND 1000),
  type TEXT NOT NULL DEFAULT 'MCQ' CHECK (type IN ('MCQ', 'TRUE_FALSE', 'SHORT_ANSWER', 'ESSAY')),
  ...
  UNIQUE (grade_level, question_number)
);

CREATE INDEX IF NOT EXISTS idx_quiz_bank_questions_grade_subject
  ON quiz_bank_questions (grade_level, subject_id, difficulty, question_number);

DROP INDEX IF EXISTS idx_classes_school_grade_name;

WITH supported_grades(grade_level, position) AS (
  VALUES
    ('Grade 4', 1),
    ('Grade 6', 2),
    ('Grade 9', 3),
...
all_strands AS (
  ...
  JOIN supported_grades g ON g.grade_level = cs.grade_level
...
WHERE c.school_id = '11111111-1111-4111-8111-111111111111'
  AND c.grade_level IN ('Grade 4', 'Grade 6', 'Grade 9', 'Form 4')
ON CONFLICT (class_id, teacher_id) DO NOTHING;

WITH supported_grades(grade_level, grade_position) AS (
  VALUES
    ('Grade 4', 1),
    ('Grade 6', 2),
    ('Grade 9', 3),
...
seed_users AS (
  ...
  SET school_id = EXCLUDED.school_id,
      ...
      updated_at = NOW()
  RETURNING id, grade_level, email
),
all_seed_students AS (
  SELECT u.id, u.grade_level, u.email
  FROM users u
  WHERE u.email LIKE 'grade%.student%@students.kitabu.ai'
     OR u.email LIKE 'form4.student%@students.kitabu.ai'
)
INSERT INTO user_roles (user_id, role)
SELECT id, 'student'::user_role
FROM all_seed_students
ON CONFLICT (user_id, role) DO NOTHING;

UPDATE users
SET password_hash = '$2b$12$VY6IMSidKcYsjbjyjfz9JuQxS1eQljjCZn8dtBnMesp8d74SJfjIm',
    must_rotate_password = TRUE,
    updated_at = NOW()
WHERE (email LIKE 'grade%.student%@students.kitabu.ai'
   OR email LIKE 'form4.student%@students.kitabu.ai')
  AND password_hash = '$2b$12$MWSjrQjGgOlUwrmLCxQM5e06zQwbMEf1gHd1A2BDkWsx.Ow4fhbLW';
...
ranked_sub_strands AS (
  ...
  WHERE cs.grade_level IN ('Grade 4', 'Grade 6', 'Grade 9', 'Form 4')
)
