...
const tree = [
  { type: 'blob', path: 'icons/filled/heart.svg' }, { type: 'blob', path: 'icons/arrow-left.svg' },
  { type: 'blob', path: 'icons/README.md' }, { type: 'blob', path: 'icons/filled/heart.png' },
  { type: 'blob', path: 'icons/filled/nested/deeper.svg' },
...
test('Tabler discovers only official SVG icon directories with MIT UI metadata', async () => {
  ...
  assert.deepEqual(discovered.assets.map(asset => asset.providerAssetId), ['icons/arrow-left.svg', 'icons/filled/heart.svg']);
...
test('Tabler has stable cursor and limit behavior', async () => {
  ...
  assert.equal(first.assets[0]?.providerAssetId, 'icons/arrow-left.svg');
  assert.equal(second.assets[0]?.providerAssetId, 'icons/filled/heart.svg');
});
