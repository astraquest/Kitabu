...
function isTechnicalGeneratedPage(page: BookPage) {
  ...
  const hasGeneratedTitlePageCopy =
    content.includes('learner book') &&
    content.includes('mascot:') &&
    content.includes('this kitabu quest book turns the curriculum');

  return (
    title === 'title page' ||
    title === 'ukurasa wa kichwa' ||
    title === 'source map' ||
    title === 'source status' ||
    title === 'source metadata' ||
    title === 'book metadata' ||
    ((pageType === 'front-matter' || title === 'title page' || title === 'ukurasa wa kichwa') &&
      (hasLegacyDeferredCoverCopy || hasGeneratedTitlePageCopy))
  );
}
...
function visiblePagesForUser(user: AuthenticatedUser, pages: BookPage[]) {
  return pages
    ...
    .filter(page => !isTechnicalGeneratedPage(page));
}
...
function hasAllGeneratedBookAccess(user: AuthenticatedUser) {
  return user.roles.some(role => role === 'teacher' || role === 'school_admin' || role === 'platform_admin');
}
...
function userCanAccessGeneratedBook(user: AuthenticatedUser, manifest: BookManifest) {
  return (
    ...
    (
      ...
      gradeToCode(manifest.grade) === gradeToCode(user.grade)
    )
  );
}
