...
test('builds a deterministic preview bundle for all 50 registered components', () => {
  ...
  const componentIds = first.scenes.map(scene => (scene as { component: { componentId: string } }).component.componentId);
  ...
  assert.equal(first.scenes.length, 50);
  ...
  const ranks = first.scenes.map(scene => {
