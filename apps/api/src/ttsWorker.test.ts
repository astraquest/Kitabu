...
import { processCartesiaQueue, processGeminiTtsQueue, processTtsJobs, type QueueProcessorDependencies } from './ttsWorker.js';
...
test('bounded Cartesia processing leaves an unavailable job assigned to Cartesia', async () => {
  const releasedProviders: string[] = [];
  const state = dependencies({
    claim: async () => [job('parent-only')],
    synthesize: async () => {
      throw new TtsProviderError({ provider: 'cartesia', kind: 'unavailable', message: 'network unavailable' });
    },
    release: async (_job, _message, _delay, _worker, provider) => { releasedProviders.push(provider); }
  });
  const result = await processCartesiaQueue({ workerId: 'worker-1', dependencies: state.dependencies, fallbackToGemini: false });
  assert.equal(result.deferred, 1);
  assert.equal(result.completed, 0);
  assert.deepEqual(releasedProviders, ['cartesia']);
});
