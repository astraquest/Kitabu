...
Local filesystem storage is the explicit development default. For a production object-store/CDN integration, set `KITABU_TTS_STORAGE_BACKEND=http-put`, provide a presigned upload URL template containing `{key}`, and set `KITABU_TTS_STORAGE_PUBLIC_BASE_URL` for reads. The adapter uploads bytes with HTTP PUT and does not log upload URLs, keys, or provider credentials.
