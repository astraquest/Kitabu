...
const kitabuGrade6SchoolLabel =
  'Choose Kitabu Demo School, Nairobi, 40 Grade 6 learners';
// Teachers pick their school before a grade is known, so the roster shows the school total.
const kitabuTeacherSchoolLabel =
  'Choose Kitabu Demo School, Nairobi, 120 learners';
const kitabuGrade8SchoolLabel =
  'Choose Kitabu Demo School, Nairobi, No Grade 8 learners yet';
const otherGrade6SchoolLabel =
  'Choose Other Demo School, Mombasa, 20 Grade 6 learners';
const kisiiGrade6SchoolLabel =
  'Choose Kisii Demo School, Kisii County, Kenya, 18 Grade 6 learners';
const defaultOnboardingSubjectIds = SUBJECTS.slice(0, 5).map(
  subject => subject.id,
);
...
function summaryCompleteMarks(root: ReactTestRenderer.ReactTestInstance) {
  return root.findAll(
    node =>
      node.type === View && node.props.testID === 'setup-summary-complete',
  );
}
...
function selectedSchoolChecks(root: ReactTestRenderer.ReactTestInstance) {
  return root.findAll(
    node => node.type === View && node.props.testID === 'selected-school-check',
  );
}
...
function missingSchoolLinks(root: ReactTestRenderer.ReactTestInstance) {
  return root.findAll(
    node =>
      node.props.testID === 'missing-school-link' &&
...
async function pressAutoAdvanceChoice(
...
async function selectCounty(
  ...
  await act(() => {
    root
      .findByProps({ accessibilityLabel: `Select ${county} county` })
      .props.onPress();
  });
}
...
test('dashboard subject grid renders without the subject picker button', async () => {
  let renderer: ReactTestRenderer.ReactTestRenderer;
...
test('student dashboard grade selector is a header dropdown and updates selection', async () => {
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Select learning grade' })
      .props.onPress();
  });

  const gradeButtonStyle = StyleSheet.flatten(
    renderer!.root
      .findByProps({ accessibilityLabel: 'Select learning grade' })
      .props.style({ pressed: false }),
  );
  const aiTextStyle = StyleSheet.flatten(
    renderer!.root.findByProps({ children: '.AI' }).props.style,
  );
  const text = renderedText(renderer!.root);

  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Selected country flag' }),
  ).toBeTruthy();
  expect(text).not.toContain('\uD83C\uDDF0\uD83C\uDDEA');
  expect(gradeButtonStyle.borderRadius).toBe(9);
  expect(gradeButtonStyle.height).toBe(32);
  expect(gradeButtonStyle.minWidth).toBe(101);
  expect(aiTextStyle.color).toBe('#DC2626');
  [
    'Grade 4',
    'Grade 5',
    'Grade 6',
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Select Form 4' })
      .props.onPress();
  });
...
test('sign-in page renders account-type cards before credentials', async () => {
  ...
  expect(
    renderer!.root.findByProps({ children: 'Choose your role' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue as Student' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue as Teacher' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue as Parent' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findAllByProps({ accessibilityLabel: 'Continue as Other' }),
  ).toHaveLength(0);
...
test('onboarding full intro captures profile details before account setup', async () => {
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Unaweza kubadilisha baadaye · You can change this later',
  );
  expect(
    renderer!.root.findAllByProps({
  ...
  expect(
    renderer!.root.findAllByProps({ testID: 'onboarding-mascot-motion' }),
  ).toHaveLength(0);
  expect(
  ...
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Select Kiswahili language',
    }).props.accessibilityState,
  ).toEqual({
    checked: false,
  });
  ...
  expect(
    renderer!.root.findAllByProps({ testID: 'onboarding-footer' }),
  ).toHaveLength(0);
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Back to language' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findAllByProps({ testID: 'onboarding-mascot-motion' }),
  ).toHaveLength(0);
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Choose Rafiki the Lion mascot',
    }).props.accessibilityState,
  ).toEqual({
    checked: false,
  });
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Choose Rafiki the Rabbit mascot',
    }).props.accessibilityState,
  ).toEqual({
    checked: false,
  });
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Choose Rafiki the Elephant mascot',
    }).props.accessibilityState,
  ).toEqual({
    checked: false,
  });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Back to language' })
      .props.onPress();
  });
  expect(renderedText(renderer!.root)).toContain('Mwalimu wako wa nyumbani');
  await pressAutoAdvanceChoice(renderer!.root, 'Select Kiswahili language');
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Choose Rafiki the Lion mascot' })
      .props.onPress();
  });
  await act(() => {
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Back to mascot' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findAllByProps({ testID: 'onboarding-mascot-motion' }),
  ).toHaveLength(0);
  expect(
    renderer!.root.findAllByProps({ accessibilityLabel: 'Back in setup' }),
  ).toHaveLength(0);
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.onPress();
  });
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Mascot navigation bar' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findAllByProps({ accessibilityLabel: 'Mascot coach tip' }),
  ).toHaveLength(0);
  expect(
    renderer!.root.findAllByProps({ testID: 'mascot-nav-back' }).length,
  ).toBeGreaterThan(0);
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
  ...
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Selected Mwanafunzi role',
    }).props.accessibilityState,
  ).toEqual({
    checked: true,
    disabled: false,
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Locked Mwalimu role' })
      .props.accessibilityState,
  ).toEqual({
    checked: false,
    disabled: true,
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Sauti ya mwalimu wako isikike vipi?',
  );
  expect(renderedText(renderer!.root)).toContain('Chagua sauti');
  expect(renderedText(renderer!.root)).toContain(
    'Gusa sauti ili kuisikiza kwanza',
  );
  ...
  expect(
    StyleSheet.flatten(
      renderer!.root.findByProps({ testID: 'voice-orb' }).props.style,
    )?.backgroundColor,
  ).toBe('#FEF0D9');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Selected voice name' })
      .props.children,
  ).toBe('Chagua sauti');
  expect(
    renderer!.root.findByProps({
  ...
  expect(
    renderer!.root.findByProps({ testID: 'voice-slider-dot-Samora' }).props
      .style,
  ).toEqual(
    expect.arrayContaining([
      expect.not.objectContaining({ backgroundColor: '#E07B00' }),
    ]),
  );
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Choose Samora voice' })
      .props.accessibilityState,
  ).toEqual({
    checked: false,
  });
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.accessibilityState,
  ).toEqual({ disabled: true, busy: false });
  expect(
    StyleSheet.flatten(
      renderer!.root.findByProps({
        accessibilityLabel: 'Continue account setup',
      }).props.style,
    ),
  ).toEqual(
  ...
  await act(async () => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Choose Jabali voice' })
      .props.onPress();
  ...
  expect(
    renderer!.root.findByProps({ testID: 'voice-slider-dot-Jabali' }).props
      .style,
  ).toEqual(
    expect.arrayContaining([
      expect.objectContaining({ backgroundColor: '#E07B00' }),
    ]),
  );

  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Use text only' })
      .props.onPress();
  });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Use text only' }).props
      .accessibilityState,
  ).toEqual({
    checked: true,
  });
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Unahitaji nini zaidi sasa hivi?',
  );
  expect(renderedText(renderer!.root)).toContain('Ili nikujue \uD83D\uDC47');
  expect(renderedText(renderer!.root)).toContain('Nina mtihani karibu');
  expect(
    renderer!.root.findByProps({ testID: 'onboarding-mascot-motion' }).props
      .accessibilityLabel,
  ).toBe('Rafiki the Lion mascot, think pose');
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Mwalimu wako wa Kitabu AI atakujua kwa jina lako.',
  );
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Your name' }).props
      .placeholder,
  ).toBe('Andika jina lako...');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
  ).toEqual({
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Tunabadilisha maudhui kulingana na umri wako.',
  );
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Your age' }).props
      .placeholder,
  ).toBe('Umri wako...');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Your age' })
      .props.onChangeText('3');
  });
  expect(renderedText(renderer!.root)).toContain(
    'Uko Sure? Age must be between 4 and 20.',
  );
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.accessibilityState,
  ...
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Select Mgeni wa Nje ya Dunia',
    }).props.accessibilityState,
  ).toEqual({
    checked: false,
  });
  expect(
    renderer!.root.findAllByProps({
      accessibilityLabel: 'Select Endelea bila kuweka',
    }),
  ).toHaveLength(0);
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Select Mgeni wa Nje ya Dunia' })
      .props.onPress();
  });
  expect(renderedText(renderer!.root)).toContain(
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Select Msichana' })
      .props.onPress();
  });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Select Msichana' }).props
      .accessibilityState,
  ).toEqual({
    checked: true,
  });
  ...
  ).toBe('Choose a school before continuing');
  await selectCounty(renderer!.root);
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Search school by name' })
      .props.onChangeText('Nia Academy');
  });
  expect(
    renderer!.root.findAllByProps({
      accessibilityLabel: 'Add Nia Academy school',
    }),
  ).toHaveLength(0);
  expect(missingSchoolLinks(renderer!.root)).toHaveLength(1);
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Clear school search' })
      .props.onPress();
  });
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: kitabuTeacherSchoolLabel })
      .props.onPress();
  });
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Select Grade 6' })
      .props.onPress();
  });
  expect(renderedText(renderer!.root)).toContain('Shule ya Msingi - Juu (CBC)');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Selected grade band' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.accessibilityState,
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Add Mathematics' })
      .props.onPress();
  });
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Add English' })
      .props.onPress();
  });
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Ninapata wasiwasi wakati wa mitihani',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Natamani ningekuwa na revision partner',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Notes zinaweza kuchosha kusoma',
  );
  expect(
    renderer!.root.findByProps({ testID: 'onboarding-mascot-motion' }).props
      .accessibilityLabel,
  ).toBe('Rafiki the Lion mascot, worried pose');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
  ).toEqual({
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Kitabu hukupa mpango wazi wa masomo na karatasi za revision',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Rafiki ni revision partner wako anayepatikana kila wakati',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Tunakuonyesha maeneo dhaifu na jinsi ya kuyarekebisha',
  );
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Kitabu AI itahakikisha hakuna dakika inayopotea.',
  );
  expect(renderedText(renderer!.root)).toContain('89%');
  expect(renderedText(renderer!.root)).toContain(
    'wanafunzi wanasema wanafanya zaidi kwa muda mfupi zaidi',
  );
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Kazi za nyumbani zinachukua muda mwingi.',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Nataka alama bora, sijui jinsi.',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Ninasahau haraka ninachojifunza.',
  );
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
  ...
  await pressAutoAdvanceChoice(
    renderer!.root,
    'Choose concern Mitihani inanisumbua sana.',
  );
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Badilisha alama mbaya kuwa nzuri.',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Maliza kazi haraka (muda zaidi wa michezo).',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Ingia chuo kikuu ninachotaka.',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Elewa vizuri ninachojifunza.',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Jisikie imara zaidi darasani.',
  );
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
  ...
  await pressAutoAdvanceChoice(
    renderer!.root,
    'Choose achievement Shangilia mitihani inayokuja.',
  );
  ...
  expect(renderedText(renderer!.root)).not.toContain(
    'Use sports examples in practice.',
  );
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Add interest Technology',
    }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Add interest Movies' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Add interest Cooking' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Skip this step' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.accessibilityState,
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Add interest Football' })
      .props.onPress();
  });
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Add interest Gaming' })
      .props.onPress();
  });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Lengo lako limegeuzwa kuwa mpango wa kila siku.',
  );
  expect(renderedText(renderer!.root)).toContain('Anza kuongeza alama');
  expect(
    renderer!.root.findByProps({
      accessibilityLabel:
        'upper-primary girl student studying in a bright studio scene',
    }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
  ...
  expect(
    renderer!.root.findByProps({ testID: 'onboarding-mascot-motion' }).props
      .accessibilityLabel,
  ).toBe('Rafiki the Lion mascot, sleep pose');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
  ).toEqual({
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Nia, mtihani wako wa Hesabu ni kesho. \uD83D\uDE80',
  );
  ...
  expect(
    StyleSheet.flatten(
      renderer!.root.findByProps({ testID: 'reminder-phone-mockup' }).props
        .style,
    )?.backgroundColor,
  ).toBe('#FBF8F3');
  expect(
  ...
  expect(
    renderer!.root.findByProps({ testID: 'onboarding-mascot-motion' }).props
      .accessibilityLabel,
  ).toBe('Rafiki the Lion mascot, cool pose');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
  ).toEqual({
  ...
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Rafiki the Lion mascot',
    }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Rafiki the Lion mascot loading avatar',
    }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Profile build progress' })
      .props.accessibilityValue,
  ).toEqual({
    max: 100,
    min: 0,
  ...
  expect(renderedText(renderer!.root)).not.toContain(
    'Nia, Profaili yako ya masomo iko tayari!',
  );
  expect(renderedText(renderer!.root)).toContain(
    'Nia,Mpango wako wa masomo uko tayari.',
  );
  expect(renderedText(renderer!.root)).toContain('Malengo ya kufikiwa');
  expect(renderedText(renderer!.root)).toContain(
    'Grade 6 \u00B7 2 masomo \u00B7 Kenya CBC',
  );
  expect(renderedText(renderer!.root)).toContain('4.89');
  expect(renderedText(renderer!.root)).toContain(
    'Kitabu ilinisaidia kupanda daraja moja kwa term.',
  );
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Show testimonial 4' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Show testimonial 1' })
      .props.accessibilityState,
  ).toEqual({
    selected: true,
  });
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Maswali ya KNEC sasa ni rahisi kufuata.',
  );
  expect(renderedText(renderer!.root)).toContain('Brian - Grade 10');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Show testimonial 2' })
      .props.accessibilityState,
  ).toEqual({
    selected: true,
  });
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Show testimonial 4' })
      .props.onPress();
  });
  expect(renderedText(renderer!.root)).toContain(
    'Rafiki hunikumbusha kusoma bila pressure.',
  );
  ...
  expect(
    renderer!.root.findAllByProps({ accessibilityLabel: 'Signup progress' }),
  ).toHaveLength(0);

  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Continue with phone number' })
      .props.onPress();
  });

  expect(renderedText(renderer!.root)).toContain('Phone number');
  expect(
    renderer!.root.findByProps({ testID: 'signup-progress-dot-1' }).props
      .accessibilityState,
  ).toEqual({
    selected: true,
  });
  expect(
    renderer!.root.findByProps({ testID: 'signup-progress-dot-2' }).props
      .accessibilityState,
  ).toEqual({
    selected: false,
  });
  expect(
    renderer!.root.findByProps({ testID: 'signup-progress-dot-3' }).props
      .accessibilityState,
  ).toEqual({
    selected: false,
  });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Send verification code' })
      .props.accessibilityState,
  ).toEqual({ disabled: true });

  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Signup phone number' })
      .props.onChangeText('0712345678');
  });
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Signup password' })
      .props.onChangeText('strongpass');
  });
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Confirm signup password' })
      .props.onChangeText('strongpass');
  });
  ...
  await act(async () => {
    await renderer!.root
      .findByProps({ accessibilityLabel: 'Send verification code' })
      .props.onPress();
  });
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Verify and continue' })
      .props.accessibilityState,
  ).toEqual({ disabled: true });
  for (let index = 1; index <= 6; index += 1) {
    const otpBox = renderer!.root.findByProps({
      accessibilityLabel: `OTP digit ${index}`,
    });
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'OTP digit 2' })
      .props.onChangeText('7');
  });
  expect(
    [1, 2, 3, 4, 5, 6].map(
      index =>
        renderer!.root.findByProps({ accessibilityLabel: `OTP digit ${index}` })
          .props.value,
    ),
  ).toEqual(['', '7', '', '', '', '']);
  await act(() => {
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'OTP digit 1' })
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Resend verification code' })
      .props.onPress();
  });
  expect(
    [1, 2, 3, 4, 5, 6].map(
  ...
  for (const [index, digit] of ['1', '2', '3', '4', '5', '6'].entries()) {
    await act(() => {
      renderer!.root
        .findByProps({ accessibilityLabel: `OTP digit ${index + 1}` })
        .props.onChangeText(digit);
    });
  }

  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Verify and continue' })
      .props.onPress();
  });
  ...
  expect(
    renderer!.root.findAllByProps({ testID: 'onboarding-dashboard' }),
  ).toHaveLength(0);
...
test('onboarding displays Form aliases and filters subjects by selected senior grade', async () => {
  ...
  await pressAutoAdvanceChoice(
    renderer!.root,
    'Choose I have an exam coming up',
  );
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Your name' })
      .props.onChangeText('55');
  });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Your name' }).props.value,
  ).toBe('');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.accessibilityState,
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Select Form 4 · KNEC' })
      .props.onPress();
  });
...
test('onboarding need intro uses teacher and parent priorities', async () => {
  ...
  for (const expectation of expectations) {
    ...
    expect(
      renderer!.root.findByProps({
        accessibilityLabel: `Selected ${
          expectation.role === 'teacher' ? 'Teacher' : 'Parent'
        } role`,
      }),
    ).toBeTruthy();
    await pressAutoAdvanceChoice(
    ...
    await pressAutoAdvanceChoice(
      renderer!.root,
      `Choose ${expectation.selectedNeed}`,
    );
    ...
    expect(
      renderer!.root.findByProps({ accessibilityLabel: 'Your name' }).props
        .placeholder,
    ).toBe(expectation.namePlaceholder);
    expect(
      renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
        .props.accessibilityValue,
    ).toEqual({
    ...
    await act(() => {
      renderer!.root
        .findByProps({ accessibilityLabel: 'Your name' })
        .props.onChangeText(expectation.name);
    });
    await act(() => {
      renderer!.root
        .findByProps({ accessibilityLabel: 'Continue account setup' })
    ...
    if (expectation.role === 'teacher') {
      ...
      await act(() => {
        renderer!.root
          .findByProps({ accessibilityLabel: 'Add teaching Grade 6' })
          .props.onPress();
      });
      ...
      await act(() => {
        renderer!.root
          .findByProps({ accessibilityLabel: 'Child name' })
          .props.onChangeText('Amani');
      });
      expect(
        renderer!.root.findByProps({
          accessibilityLabel: 'Continue account setup',
      ...
      await act(() => {
        renderer!.root
          .findByProps({ accessibilityLabel: 'Child age' })
          .props.onChangeText('3');
      });
      expect(
        renderer!.root.findByProps({
      ...
      await act(() => {
        renderer!.root
          .findByProps({ accessibilityLabel: 'Child age' })
          .props.onChangeText('11');
      });
      await act(() => {
        renderer!.root
          .findByProps({ accessibilityLabel: 'Select child Grade 6' })
          .props.onPress();
      });
      expect(
        renderer!.root.findByProps({
      ...
      await act(() => {
        renderer!.root
          .findByProps({ accessibilityLabel: 'Add another child' })
          .props.onPress();
      });
      expect(
        renderer!.root.findByProps({ accessibilityLabel: 'Child 2 name' }),
      ).toBeTruthy();
      await act(() => {
        renderer!.root
          .findByProps({ accessibilityLabel: 'Child 2 name' })
          .props.onChangeText('Baraka');
      });
      await act(() => {
        renderer!.root
          .findByProps({ accessibilityLabel: 'Child 2 age' })
          .props.onChangeText('10');
      });
      await act(() => {
        renderer!.root
          .findByProps({ accessibilityLabel: 'Select child 2 Grade 4' })
          .props.onPress();
      });
      expect(
        renderer!.root.findByProps({
      ...
      expect(
        renderer!.root.findByProps({ accessibilityLabel: 'Child 3 name' }),
      ).toBeTruthy();
      expect(
        renderer!.root.findAllByProps({
...
test('onboarding role step does not expose the removed Other role', async () => {
  ...
  expect(
    renderer!.root.findAllByProps({
      accessibilityLabel: 'Selected Other role',
    }),
  ).toHaveLength(0);
  expect(
    renderer!.root.findAllByProps({ accessibilityLabel: 'Choose Other role' }),
  ).toHaveLength(0);
  expect(renderedText(renderer!.root)).not.toContain('Other');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Select Alien from space' })
      .props.onPress();
  });
  expect(renderedText(renderer!.root)).toContain(
  ...
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Select Alien from space',
    }).props.accessibilityState,
  ).toEqual({
    checked: false,
  });
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Select Grade 7' })
      .props.onPress();
  });
  await act(() => {
    renderer!.root
...
test('full intro loading and ready states use teacher and parent context', async () => {
  ...
  const expectations = [
    {
      ...
      detailAction: async (root: ReactTestRenderer.ReactTestInstance) => {
        ...
        await act(() => {
          root
            .findByProps({ accessibilityLabel: 'Add teaching Grade 7' })
            .props.onPress();
        });
      },
    ...
    },
    {
      ...
      detailAction: async (root: ReactTestRenderer.ReactTestInstance) => {
        ...
        expect(
          root.findByProps({ accessibilityLabel: 'Continue account setup' })
            .props.accessibilityState,
        ).toEqual({
          disabled: true,
          busy: false,
        ...
        await act(() => {
          root
            .findByProps({ accessibilityLabel: 'Select child 2 Grade 5' })
            .props.onPress();
        });
        expect(
          root.findByProps({ accessibilityLabel: 'Continue account setup' })
  ...
  for (const expectation of expectations) {
    ...
    if (expectation.role === 'teacher') {
      ...
      expect(
        renderer!.root.findByProps({ accessibilityLabel: 'Add Mathematics' })
          .props.accessibilityState,
      ).toEqual({
        disabled: false,
        selected: false,
      ...
      ).toBe('Choose a school before continuing to child details');
      await selectCounty(renderer!.root);
      await act(() => {
        renderer!.root
          .findByProps({ accessibilityLabel: kitabuTeacherSchoolLabel })
      ...
      expect(renderedText(renderer!.root)).toContain(
        'Which subjects should we track for Amani?',
      );
      expect(
        renderedText(
          renderer!.root.findByProps({
      ...
      expect(renderedText(renderer!.root)).toContain(
        'Which subjects should we track for Baraka?',
      );
      expect(
        renderedText(
          renderer!.root.findByProps({
    ...
    if (expectation.role === 'teacher') {
      ...
      expect(renderedText(renderer!.root)).toContain(
        'Too much marking and admin work.',
      );
      expect(renderedText(renderer!.root)).toContain(
        "Hard to identify each student's weak areas.",
      );
      expect(renderedText(renderer!.root)).toContain(
        'Struggling to cover the full syllabus.',
      );
      expect(renderedText(renderer!.root)).toContain(
        'Students underperforming in exams.',
      );
    } else {
      expect(renderedText(renderer!.root)).toContain(
      ...
      expect(renderedText(renderer!.root)).toContain(
        'Too much time on phone/TV instead of studying.',
      );
      expect(renderedText(renderer!.root)).toContain(
        "They don't understand what they're taught.",
      );
      expect(renderedText(renderer!.root)).toContain(
        'Homework is a constant battle at home.',
      );
      expect(renderedText(renderer!.root)).toContain(
        "I don't know how to help them at home.",
      );
    }
    await pressAutoAdvanceChoice(
      renderer!.root,
      `Choose concern ${expectation.concern}`,
    );
    if (expectation.role === 'teacher') {
      expect(renderedText(renderer!.root)).toContain(
      ...
      expect(renderedText(renderer!.root)).toContain(
        'Save at least 3 hours per week on prep.',
      );
      expect(renderedText(renderer!.root)).toContain(
        'Make every lesson more engaging.',
      );
      expect(renderedText(renderer!.root)).toContain(
        'Identify weak students early and help them.',
      );
      expect(renderedText(renderer!.root)).toContain(
        'Complete the full syllabus on time.',
      );
    } else {
      expect(renderedText(renderer!.root)).toContain(
      ...
      expect(renderedText(renderer!.root)).toContain(
        'Build a daily study habit for them.',
      );
      expect(renderedText(renderer!.root)).toContain(
        "Understand exactly where they're struggling.",
      );
      expect(renderedText(renderer!.root)).toContain(
        'Set them up for university success.',
      );
      expect(renderedText(renderer!.root)).toContain(
        'Reduce exam stress and anxiety for them.',
      );
    }
    await pressAutoAdvanceChoice(
      renderer!.root,
      `Choose achievement ${expectation.achievement}`,
    );
    ...
    if (expectation.role === 'teacher') {
      ...
      expect(renderedText(renderer!.root)).toContain(
        'Turn Grade 6 + 1 more practice into short CBC drills',
      );
      expect(renderedText(renderer!.root)).toContain('Mathematics');
      expect(renderedText(renderer!.root)).toContain('English');
      expect(renderedText(renderer!.root)).toContain(
      ...
      expect(renderedText(renderer!.root)).toContain(
        'Low engagement? Kitabu AI turns lessons into interactive quizzes and games.',
      );
      expect(renderedText(renderer!.root)).toContain('Start Lifting My Class');
      expect(
        renderer!.root.findByProps({
          accessibilityLabel: 'teacher Good News plan',
        }),
      ).toBeTruthy();
    } else {
      ...
      expect(renderedText(renderer!.root)).toContain(
        'See how 2 children are doing in Grade 6',
      );
      expect(renderedText(renderer!.root)).toContain('Mathematics');
      expect(renderedText(renderer!.root)).toContain('English');
      expect(renderedText(renderer!.root)).toContain(
      ...
      expect(renderedText(renderer!.root)).toContain(
        'When motivation drops, Kitabu AI turns revision into short wins your child can finish daily.',
      );
      expect(renderedText(renderer!.root)).toContain('Start Tracking Progress');
      expect(
        renderer!.root.findByProps({
          accessibilityLabel: 'parent Good News plan',
        }),
      ).toBeTruthy();
    }
...
test('teacher onboarding uses teacher copy and submits school, class, and optional M-Pesa', async () => {
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.accessibilityHint,
  ).toBe('Moves to school selection');
  expect(renderer!.root.findAllByType(ChevronRight)).toHaveLength(0);
  expect(summaryCompleteMarks(renderer!.root)).toHaveLength(1);
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Rafiki the Lion teacher mascot',
    }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Select Grade 6' }).props
      .accessibilityState,
  ).toEqual({ checked: true });

  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Select Grade 8' })
      .props.onPress();
    renderer!.root
      .findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.onPress();
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'School selector' }).props
      .accessibilityState,
  ).toEqual({
    disabled: true,
  });
  expect(
    renderedText(
      renderer!.root.findByProps({ accessibilityLabel: 'School selector' }),
    ),
  ).toContain('Select county first');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.accessibilityState,
  ).toEqual({ disabled: true, busy: false });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.accessibilityHint,
  ).toBe('Choose a county before continuing');
  expect(renderedText(renderer!.root)).toContain('Back to class');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Back in setup' }).props
      .accessibilityHint,
  ).toBe('Returns to the first setup step');
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: kitabuGrade8SchoolLabel })
      .props.onPress();
  });
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Final setup review' })
      .props.accessibilityValue,
  ).toEqual({
    text: 'Class setup ready. Main class: Grade 8. School: Kitabu Demo School. Payment: Skip for now.',
  });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Finish account setup' })
      .props.accessibilityHint,
  ).toBe('Completes account setup without adding M-Pesa');
  expect(renderedText(renderer!.root)).toContain('Back to school');
  const backToSchoolText = renderer!.root
    .findAllByType(Text)
    .find(node => node.props.children === 'Back to school');
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Finish account setup' })
      .props.onPress();
  });
...
test('parent onboarding uses family dashboard copy', async () => {
  ...
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Rafiki the Elephant parent mascot',
    }),
  ).toBeTruthy();
});
...
test('onboarding setup summary updates across role choices', async () => {
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Teacher setup summary' })
      .props.accessibilityValue,
  ).toEqual({
  ...
  expect(summaryCompleteMarks(renderer!.root)).toHaveLength(1);
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Mascot coach tip' }).props
      .children,
  ).toBe('Choose class');
  ...
  expect(
    setupSummaryValues(renderer!.root).find(
      node => node.props.children === 'Kitabu Demo School',
    )?.props.numberOfLines,
  ).toBe(2);
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Continue account setup' })
      .props.accessibilityHint,
  ).toBe('Moves to the optional payment step');
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'M-Pesa number' })
      .props.onChangeText('0716175485');
  });
...
test('onboarding uses compact review rows without truncating payment details', async () => {
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: kitabuGrade6SchoolLabel })
      .props.onPress();
  });
  await act(() => {
    renderer!.root
  ...
  const reviewValues = renderer!.root
    ...
    .findAllByType(Text)
    .filter(node =>
      ['Grade 6', 'Kitabu Demo School', 'Skip for now'].includes(
        node.props.children,
      ),
    );
...
test('onboarding validates and normalizes optional M-Pesa numbers before submit', async () => {
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'M-Pesa number' }).props
      .accessibilityHint,
  ).toBe('Optional Safaricom number for faster checkout later');
  await act(() => {
    renderer!.root
      .findByProps({ placeholder: '2547XXXXXXXX' })
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Family setup summary' })
      .props.accessibilityValue,
  ).toEqual({
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Finish account setup' })
      .props.accessibilityHint,
  ).toBe('Checks the M-Pesa number before finishing setup');
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Finish account setup' })
      .props.onPress();
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Back in setup' })
      .props.onPress();
  });
...
test('student onboarding exposes accessible gender and grade selections', async () => {
  ...
  const girl = renderer!.root.findByProps({
    accessibilityLabel: 'Select Girl',
  });
  const skip = renderer!.root.findByProps({
    accessibilityLabel: 'Select Skip',
  });
  ...
  expect(renderedText(renderer!.root)).toContain(
    'Pick up to five CBC subjects',
  );
  const mascot = renderer!.root.findByProps({
    accessibilityLabel: 'Rafiki the Rabbit student mascot',
  });
  expect(mascot).toBeTruthy();
  const mascotMotion = renderer!.root.findByProps({
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Gender options' }).props
      .accessibilityRole,
  ).toBe('radiogroup');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Grade options' }).props
      .accessibilityRole,
  ).toBe('radiogroup');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Select Grade 6' }).props
  ...
  expect(
    React.Children.toArray(
      renderer!.root.findByProps({ testID: 'onboarding-footer' }).props
        .children,
    ),
  ).toHaveLength(1);
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Subjects you study' })
      .props.accessibilityRole,
  ).toBe('list');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Remove Mathematics' })
      .props.accessibilityState,
  ).toEqual({ disabled: false, selected: true });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Add Agriculture' }).props
      .accessibilityState,
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Select Girl' }).props
      .accessibilityState,
  ).toEqual({ checked: true });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Select Skip' }).props
      .accessibilityState,
  ).toEqual({ checked: false });

  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Remove Social Studies' })
      .props.onPress();
  });
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Add Agriculture' })
      .props.onPress();
  });

  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Remove Agriculture' })
      .props.accessibilityState,
  ).toEqual({ disabled: false, selected: true });
});
...
test('onboarding supports keyboard submit for school search and M-Pesa', async () => {
  ...
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Select Nairobi City county',
    }).props.accessibilityState,
  ).toEqual({
    checked: false,
  });
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Select Baringo county' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({
      accessibilityLabel: 'Select West Pokot county',
    }),
  ).toBeTruthy();
  ...
  const countyLabels = new Set(countyOptionLabels);
  expect(countyLabels.size).toBe(47);
  expect(missingSchoolLinks(renderer!.root)).toHaveLength(0);
  await selectCounty(renderer!.root);
  // Picking a county opens the searchable school dropdown.
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Search school by name' })
      .props.editable,
  ).toBe(true);
  const schoolResults = renderer!.root.findByProps({
  ...
  expect(
    renderedText(
      renderer!.root.findByProps({ accessibilityLabel: 'County selector' }),
    ),
  ).toContain('Nairobi City');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'School result count' })
      .props.children,
  ).toBe('Showing 1 school in Nairobi City for Grade 6');
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Search school by name' })
      .props.onChangeText('Kitabu Demo');
  });
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Search school by name' })
  ...
  const continuePaymentText = renderer!.root
    .findAllByType(Text)
    .find(node => node.props.children === 'Continue to payment');
...
test('onboarding announces empty school search results', async () => {
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'Search school by name' })
      .props.onChangeText('No Such School');
  });

  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'School result count' })
      .props.children,
  ).toBe('No schools found in Nairobi City for Grade 6');
  ...
  expect(
    renderer!.root.findAllByProps({
      accessibilityLabel: 'Add No Such School school',
    }),
  ).toHaveLength(0);
  expect(renderedText(renderer!.root)).toContain(
...
test('onboarding opens missing-school help through WhatsApp and reports failures', async () => {
  ...
  const helpLink = renderer!.root.findByProps({
    accessibilityLabel: 'Request school on WhatsApp',
  });
  expect(helpLink.props.accessibilityRole).toBe('button');
  expect(helpLink.props.accessibilityHint).toBe(
    'Opens WhatsApp to message Kitabu admin',
  );
  ...
  expect(openUrlSpy).toHaveBeenCalledWith(
    expect.stringContaining('https://wa.me/254716175485'),
  );
  expect(openUrlSpy).toHaveBeenCalledWith(
    expect.stringContaining('Nairobi%20City'),
  );
  ...
  await act(async () => {
    await renderer!.root
      .findByProps({ accessibilityLabel: 'Request school on WhatsApp' })
      .props.onPress();
  });

  expect(renderedText(renderer!.root)).toContain(
    'Could not open WhatsApp. Message admin at 0716175485.',
  );
  expect(
    renderer!.root.findByProps({ role: 'alert' }).props.accessibilityLiveRegion,
  ).toBe('polite');
...
test('onboarding clears selected school when the search query changes', async () => {
  ...
  await act(() => {
    renderer!.root
      .findByProps({ accessibilityLabel: 'School selector' })
      .props.onPress();
  });
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Clear school search' })
      .props.accessibilityHint,
  ).toBe('Clears the school search and selected school');
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'School result count' })
      .props.children,
  ).toBe('Showing 1 school in Kisii for Grade 6');
  expect(
    renderer!.root.findByProps({ accessibilityLabel: kisiiGrade6SchoolLabel }),
  ).toBeTruthy();
...
test('onboarding dismisses keyboard during step navigation and uses mobile keyboard layout props', async () => {
  ...
  expect(
    scrollView.props.keyboardDismissMode === 'on-drag' ||
      scrollView.props.keyboardDismissMode === 'interactive',
  ).toBe(true);
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Onboarding progress' })
      .props.accessibilityValue,
...
test('onboarding selected controls use role accent colors', async () => {
  ...
  for (const expectation of expectations) {
    ...
    expect(
      StyleSheet.flatten(
        renderer!.root.findByProps({ accessibilityLabel: 'Select Grade 6' })
          .props.style,
      ).backgroundColor,
...
test('subscription modal shows Sungura Simba Ndovu packages and discounts', async () => {
  ...
  expect(textValues.indexOf('Sungura')).toBeLessThan(
    textValues.indexOf('Simba'),
  );
  ...
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Sungura mascot' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Simba mascot' }),
  ).toBeTruthy();
  expect(
    renderer!.root.findByProps({ accessibilityLabel: 'Ndovu mascot' }),
  ).toBeTruthy();
});
...
test('subscription modal can render inline without opening a native modal', async () => {
  let renderer: ReactTestRenderer.ReactTestRenderer;
  ...
  expect(renderedText(renderer!.root)).toContain('Continue to Pay - KSH 250');
});
...
test('subscription modal centers the tapped package and updates checkout amount', async () => {
  ...
  const modalProps = {
    isOpen: true,
    plans,
    selectedPlanCode: 'monthly' as const,
    phoneNumber: '',
    maskedSavedPhoneNumber: null,
    isSubmitting: false,
    statusLabel: null,
  ...
  await act(() => {
    renderer = ReactTestRenderer.create(
      <SubscriptionCheckoutModal {...modalProps} />,
    );
  });

  const ndovuButton = renderer!.root.findByProps({
    accessibilityLabel: 'Select Ndovu package',
  });
  ...
  const expandedText = renderedText(renderer!.root);
  expect(expandedText).toContain('Sungura');
  expect(expandedText).toContain('Simba');
  expect(expandedText).toContain('Ndovu');
  expect(expandedText).not.toContain('Ndovu selected');
  expect(expandedText).toContain('Continue to Pay - KSH 1,999');
  expect(onSelectPlan).toHaveBeenCalledTimes(1);

  await act(() => {
    renderer!.update(
      <SubscriptionCheckoutModal {...modalProps} isOpen={false} />,
    );
  });
  await act(() => {
    renderer!.update(
      <SubscriptionCheckoutModal {...modalProps} selectedPlanCode="annual" />,
    );
  });

  const reopenedText = renderedText(renderer!.root);
  expect(reopenedText).toContain('Sungura');
  expect(reopenedText).toContain('Simba');
  expect(reopenedText).toContain('Ndovu');
  expect(reopenedText).not.toContain('Ndovu selected');
});
