...
export const INITIAL_SUBMITTED_ASSIGNMENTS: SubmittedAssignment[] = INITIAL_ASSIGNMENTS.map(
  (assignment, index) => ({
    ...assignment,
    submittedCount: index === 2 ? 5 : Math.max(2, 5 - index),
    totalStudents: 5,
...
export const INITIAL_PARENT_CHILDREN: ParentChildSummary[] = INITIAL_TEACHER_STUDENTS.map(
  (student, index) => ({
    ...
    completed_lessons: [18, 14, 9, 16, 7][index],
    total_lessons: 20,
    mastery_average: Math.max(45, student.assessmentScore - 4),
    due_reviews: [1, 2, 4, 1, 5][index],
    ...
    weekly_report: {
      ...
      activeDays: [5, 4, 3, 5, 2][index],
      lessonsCompleted: [6, 5, 3, 6, 2][index],
      assignmentsCompleted: [4, 3, 2, 4, 1][index],
