...
export async function getKitabuDeviceId() {
  let deviceId = await loadSecureJson<string | null>(
...
export function getUserFacingApiError(
  ...
  if (
    ...
    errorPayload.error !== 'Bad Request'
  ) {
...
export async function readJsonResponse<T>(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        getUserFacingApiError({ message: text }, fallbackMessage),
      );
    }
...
export async function apiJsonRequest<T>(
  ...
  const sendRequest = async () => {
    try {
      ...
      throw new ApiRequestError({
        message:
          'Unable to connect to Kitabu. Check your connection and try again.',
  ...
  if (!response.ok) {
    ...
    throw new ApiRequestError({
      message: getUserFacingApiError(payload),
